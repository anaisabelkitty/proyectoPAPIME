// Prueba completa BLE + WiFi + UART del ESP32-C6 (basada en el firmware de la rama feature/esp32-mega)
// Guía: docs/prueba_wifi_ble_esp32_mega.md. Pareja del lado del Mega: src/mega_pruebas/wifi_ble_uart_prueba.cpp
//
...
//   Mega TX1 (pin 18) ──[divisor 5V→3.3V]──> ESP32-C6 GPIO17 (RX)
//   Mega RX1 (pin 19) ───────────────────────> ESP32-C6 GPIO16 (TX)
//   115200 baudios, 8N1. Serial1 en GPIO16/17; la consola sale por el USB nativo (CDC on boot).
//
...
// ─── UART con el Mega ──────────────────────────────────────────────
static const int  UART_RX_PIN  = 17;   // ESP32 RX  <- Mega TX1 (pin 18) por el divisor
static const int  UART_TX_PIN  = 16;   // ESP32 TX  -> Mega RX1 (pin 19)
...
void setup() {
    ...
    while (!Serial && millis() - t0 < 3000) delay(10);   // USB nativo (CDC): esperar al monitor
