...
// La identificación y lectura de los 4 conectores RJ45 vive en
// nucleo/conectores.*; el envío al ESP32 en nucleo/enlace_esp32.*; el LCD
// en nucleo/pantalla.*. Este archivo solo orquesta el modo operación y el
// menú de calibración por Serial (ver docs/diseno_electronico.md para el
// detalle de pines de los conectores).
...
// ─── Modo operación (por defecto: LCD + envío al ESP32) ───────
// El kit arranca aquí, sin botones: se refresca solo cada 500ms. Cualquier
// tecla por USB entra al menú de calibración de abajo; la opción 0 regresa.
bool modoOperacion = true;
LecturaConector g_lecturas[NUCLEO_NUM_CONECTORES];
EstadoEnlace    g_enlace;
unsigned long   g_ultimoCiclo = 0;
const unsigned long CICLO_OPERACION_MS = 500; // cadencia del JSON (docs/protocolo.md §6)
...
void imprimirMenu() {
    ...
    Serial.println("  0  Volver a operacion (LCD + envio al ESP32)");
    Serial.println("-----------------------------------------");
    Serial.print("> ");
}
...
void setup() {
    ...
    enlace_inicializar();
    enlace_reset(&g_enlace);

    pantalla_inicializar();   // si no hay LCD conectado, el resto sigue igual
    pantalla_splash();
    delay(1500);

    Serial.println(F("Kit Sensei en operacion (LCD + envio al ESP32)."));
    Serial.println(F("Pulsa cualquier tecla por USB para el menu de calibracion."));
}
...
void loop() {
    ...
    // ── Modo operación: LCD + JSON al ESP32 cada 500 ms ────────
    if (modoOperacion) {
        enlace_procesarEntrada(&g_enlace);   // #NAME / #WIFI / #IP del ESP32

        if (millis() - g_ultimoCiclo >= CICLO_OPERACION_MS) {
            g_ultimoCiclo = millis();
            conectores_escanear(g_lecturas);
            enlace_enviarJson(g_lecturas);
            pantalla_dibujar(g_lecturas, g_enlace);
        }
    ...
    if (modoActivo == 0 && !calEnCurso) {
        ...
        else if (op == 0) { modoOperacion = true; g_ultimoCiclo = 0; return; }
