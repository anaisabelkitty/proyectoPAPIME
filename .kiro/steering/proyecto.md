...
Documentación técnica completa de cada sensor: `#[[file:../Documento.md]]`
...
- **Opción A — NimBLE (recomendada):** librería BLE más ligera y eficiente en memoria. Es la recomendada para proyectos nuevos con ESP32.
- **Opción B — ESP32 BLE Arduino:** librería estándar incluida con el framework Arduino para ESP32. Más documentación disponible pero consume más memoria RAM.
- **Opción C — ESP-IDF directo:** API nativa de Espressif sin capa Arduino. Máximo control pero más compleja de implementar.
...
- Solo aparecen los sensores que están enchufados; los conectores vacíos no aparecen.
...
- `sensei`: identificador único del kit (últimos 4 caracteres de la MAC).
- `puerto`: número del conector RJ45 (1 a 4).
- `nombre`: nombre del sensor detectado.
...
PlatformIO + framework Arduino + C++. El código del Mega y el de la ESP32 están en el mismo repositorio con dos entornos separados en el `platformio.ini`.
...
│   └── main.cpp                            ⬜ pendiente
└── extras/
    ├── Documento.md                        ← documentación técnica de sensores
...
3. **Confirmar estructura del JSON** con el desarrollador de la app Android (ver `extras/android.md`).
4. **Sensores 10–16** — todavía no están definidos.
5. **Perfil BLE** (UUIDs GATT) — coordinar con el desarrollador Android (ver `extras/android.md`).
...
- Documentación técnica de sensores: `extras/Documento.md`
- Coordinación con Android: `extras/android.md`
