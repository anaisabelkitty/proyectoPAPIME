...
Código de identificación del HC-SR04: **9** (binario 1001: ID3=1, ID2=0, ID1=0, ID0=1).
...
Como el código es 9 (ID3=1, ID2=0, ID1=0, ID0=1), van dos bits en 1 (eléctricamente LOW = resistencia a GND):
...
- Pines 4 y 5 del RJ45: sin ningún cable.
...
5. Debe aparecer: `Conector 3 (codigo 9): Distancia = XX.X cm`.
...
Si el código no coincide con 9, revisar con multímetro (todo apagado): continuidad entre el pin 3 y GND (~1 kΩ), y entre el pin 6 y GND (~1 kΩ), ambas del RJ45 del módulo. Si el código sí es 9 pero la distancia da error o "sin eco", revisar que Trig y Echo no estén invertidos entre sí (pin 7 = Trig, pin 8 = Echo).
